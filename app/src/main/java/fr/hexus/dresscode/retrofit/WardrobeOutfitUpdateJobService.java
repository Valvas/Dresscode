...
public class WardrobeOutfitUpdateJobService extends JobService
{
    @Override
    public boolean onStartJob(JobParameters job)
    {
        new WardrobeOutfitUpdateJobService.DresscodeAsyncTask(this).execute(job);
    ...
    private class DresscodeAsyncTask extends AsyncTask<JobParameters, Void, JobParameters> implements IJobServiceObserver
    {
        ...
        protected JobParameters doInBackground(JobParameters... job)
        {
            ...
            Log.println(Log.INFO, Constants.LOG_NETWORK_MANAGER_UPDATING_WARDROBE_OUTFIT, "Updating wardrobe outfit in the API (" + currentOutfit.getUuid() + ")");

            currentOutfit.updateOutfitInTheApi(token, getApplicationContext());
